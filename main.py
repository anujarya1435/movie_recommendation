...
#     raise HTTPException(
#         status_code=404, detail=f"Title not dound in local dataset:'{title}' "
#     )
...
#     with open(TFIDF_MATRIX_PATH ,"rb"):
...
#         raise RuntimeError("df.pkl must contain a DateFrame with a 'title  column ")
...
#         raise HTTPException(status_code=500,detail=f"Home route falied: {e}")
...
# ):
#     """
#     Returns Raw TMDB shape with 'results 
#     list
#      Stramlit will use it for:
...
@app.get("/home", response_model=List[TMDBMovieCard])
async def home(category: str = Query("popular"), limit: int = Query(24)):
    if category == "trending":
        data = await tmdb_get("/trending/movie/day", {"language": "en-US"})
    else:
...
@app.get("/recommend/tfidf")
async def recommend_tfidf(title: str):
    recs = tfidf_recommend_titles(title)
    return [{"title": t, "score": s} for t, s in recs]
